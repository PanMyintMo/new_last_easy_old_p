...
import com.gawilive.live.adapter.LiveChatAdapter;
import com.gawilive.live.adapter.LiveUserAdapter;
...
import com.gawilive.live.custom.LiveLightView;
import com.gawilive.live.custom.TopGradual;
import com.gawilive.live.dialog.LiveUserDialogFragment;
import com.gawilive.live.http.LiveHttpConsts;
import com.gawilive.live.http.LiveHttpUtil;
import com.gawilive.live.presenter.LiveDanmuPresenter;
import com.gawilive.live.presenter.LiveEnterRoomAnimPresenter;
import com.gawilive.live.presenter.LiveGiftAnimPresenter;
import com.gawilive.live.presenter.LiveLightAnimPresenter;
...
public class LiveRoomViewHolder extends AbsViewHolder implements View.OnClickListener {
    ...
    private TextView mOnlineNum; // 在线人数
...
//    private View mPrizePoolGuang;
    ...
    public void init() {
        ...
        mOnlineNum = findViewById(R.id.online_people_num);
        //用户头像列表
...
//            }
    ...
    public void setUserList(List<LiveUserGiftBean> list) {
        if (mLiveUserAdapter != null) {
            mLiveUserAdapter.refreshList(list);
            mOnlineNum.setText(mLiveUserAdapter.getItemCount() +mContext.getString(R.string.str_005));
        }
    }
    ...
    public void startAnchorLiveTime() {
        ...
        mAnchorTimeTicker = new Runnable() {
            @Override
            public void run() {
                long now = SystemClock.uptimeMillis();
                if (mLiveTimeTextView != null) {
                    mLiveTimeTextView.setText(StringUtil.getDurationText2(mAnchorLiveTime));
                }
                mAnchorLiveTime += 1;
                if (mAnchorTimeHandler != null) {
                    long next = now + (1000 - now % 1000);
                    mAnchorTimeHandler.postAtTime(mAnchorTimeTicker, next);
                }
            }
        };
        mAnchorTimeTicker.run();
    }
